...
that the imbalance of homochiral modes results from the punctuated energy and helicity injection during the reconnection. \DEL{Crucially, the chiral imbalance occurs spontaneously from reconnections, unlike ordinary Navier-Stokes turbulence
...
$\bom^2/\bom^2_{max}$. In the third panel, note the Kelvin waves on the superfluid vortices. \NOTE{we lack a discussion of units in the text. $T=?$} \NOTE{can we show helicity?}}
...
In turbulent systems, the type and the number of sign-defined ideal invariants determine the direction of cascades. Indeed, the famous and simple Fjørtoft argument \cite{fjortoft1953changes} predicts in 2D classical turbulence the existence of an inverse energy cascade. Similarly, for 3D wave-turbulent 3D BECs, the Fjørtoft argument predicts an inverse particle and a direct energy cascades, as recently adressed theoretically \cite{Zhu_DirectInverseCascades_2023}. In 3D classical fluids, helicity, which is also an inviscid invariant, is not sign-defined and thus only a direct energy cascade is possible. However, recent studies have demonstrated that the direction of the energy 
...
mutual friction forcing at the same times as in the main figure.\NOTE{GK: units of $k$?, the inset could go as pannel (b), it would not change much the aspect ratio.}}
...
\emph{Inset:} Post reconnection evolution of the integral length scale, $\mathcal{L}_E$ \NOTE{GK:in text is $\mathcal{L}$, what are the units? maybe normalise it by its initia value?} \NOTE{GK: how is the integral scale evolution before reconnection?} \NOTE{GK: we should normalise $\Pi_E$. In general, we use the total dissipation, but here we might use the total mutual friction injection (hopefully the values are of order one...)}.
...
energy\NOTE{GK: why $\pi/2$?}. The inset of Fig.~\ref{fig:energy-flux} shows that
