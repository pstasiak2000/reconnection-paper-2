...
\begin{abstract}
Vortex reconnections play a fundamental role in fluids and they are typically considered as a mechanism to increase their complexity and to develop small scales. In this work, we show that in superfluids, they can also excite large scales.
...
$\bom^2/\bom^2_{max}$. In the third panel, note the Kelvin waves on the superfluid vortices. \NOTE{ $T=?$}}
...
% force $\mathbf{F}_{ns}$. 
\NOTE{GK: I'm not sure of understanding the meaning and need of the paragraph in green, maybe delete and go directly to the flux?}\DEL{Immediately after the reconnection, each vortex has the shape of
...
as it can be observed in Fig.~\ref{fig:kinetic-energy}.
...
due to the incompressible constraint). Finally, the helical decomposition naturally allow us decompose the total helicity as $\mathcal{H}=\mathcal{H}^+-\mathcal{H}^-$\NOTE{GK: I commented the sentences about the spectra, they were wrong and we don't use them.}.
% %
