...
where $\tau_R = L^2/\kappa$. 
The green tubes represent the superfluid vortex lines (the tubes’ radii have been 
...
\begin{figure}[t]
    ...
    % \end{subfigure}
\caption{\red{ \emph(a) Temporal evolution of the normal fluid helicity $\mathcal{H} = \int_{\mathcal{V}}H(\x)dV$ computed over the entire volume $\mathcal{V}$. \emph(b) Post reconnection evolution of the integral length scale, $L_0$ scaled by the reconnection time integral scale $L_R$. The bold solid lines represent the orthogonal reconnections (right axis). In \emph{(a)} and left-axis of \emph{(b)}, the slightly faded lines represent the data at the same temperature for Hopf-link reconnections from Ref.~\cite{stasiak2025experimental}.
...
and $T=2.1K$ (blue).\emph{(b):} Spectral normal fluid kinetic energy flux, $\Pi (k)$. \red{It is normalised using the box size and the quantum of circulation $\kappa$}. Times and temperatures are labelled as in Fig.~3a.}
\label{fig:spectra}
\end{figure}
%
...
$L_R$ indeed  increases steadily in the post-reconnection regime. Note that times have been normalised by the largest eddy-turnover-time at the reconnection event, evidencing  it fast evolution.
...
frequent non-symmetrical reconnections may be strong enough to induce an inverse energy cascade~\cite{notes_recon}.
