...
%DIF DEL initial_submission.tex   Fri May  2 21:13:48 2025
%DIF ADD main_REV.tex             Thu Jun 26 17:06:03 2025
%
...
%DIF 45c45
%DIF < \usepackage{dcolumn}% Align table columns on decimal point
%DIF -------
\usepackage{dcolumn}% Align table columns on decimal poi<?xml version="1.0" encoding="UTF-8" standalone="no"?> %DIF > 
%DIF -------
...
\author{P. Z. Stasiak}
\DIFaddbegin \affiliation{School of Mathematics, Statistics and Physics, Newcastle University, Newcastle upon Tyne, NE1 7RU, United Kingdom}

\DIFaddend \author{A. Baggaley}
\DIFaddbegin \affiliation{School of Mathematics, Statistics and Physics, Newcastle University, Newcastle upon Tyne, NE1 7RU, United Kingdom}
\affiliation{Department of Mathematics and Statistics, Lancaster University, Lancaster, LA1 4YF, UK}

\DIFaddend \author{C.F. Barenghi}
...
$(t-t_R)/\tau_R=-7.47\times10^{-2}$, $-3.09\times10^{-3}$, $8.79\times10^{-4}$ and $5.39\times10^{-3}$, where \DIFdelbeginFL \DIFdelFL{$\tau_R = E^{1/2}_R/ L_R$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\tau_R = L_R/E^{1/2}_R$. The quantities $E_R$ and $L_R$ are the energy $E$ and integral length scales $L$ of the normal fluid at reconnection time $t_R$}\DIFaddendFL . The green
...
vortex at \DIFdelbeginFL \DIFdelFL{$(t-t_0)/\tau_R=8.79\times10^{-4}$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$(t-t_R)/\tau_R=8.79\times10^{-4}$ }\DIFaddendFL . In the bottom sequence, the red/blue volume
...
\cite{canuto-dalsgaard-1998}) to quantum \DIFdelbegin \DIFdel{fluid
}\DIFdelend \DIFaddbegin \DIFadd{fluids
}\DIFaddend (quantum turbulence \cite{barenghi-etal-2023,
...
is characterised by a direct \DIFaddbegin \DIFadd{(forward) }\DIFaddend cascade: the
...
\DIFdelbegin \DIFdel{Similarly to
}\DIFdelend \DIFaddbegin \DIFadd{In this forward energy cascade a fundamental role is played by the dynamics and interactions of quantum vortices,
...
\mbox{%DIFAUXCMD
\cite{koplik-levine-1993,bewley-etal-2008,rorai-etal-2016,serafini-etal-2017,galantucci-baggaley-parker-barenghi-2019,villoisUniversalNonuniversalAspects2017,villois2020irreversible}}\hspace{0pt}%DIFAUXCMD
.
...
and triggering the Kelvin-wave cascade \mbox{%DIFAUXCMD
\cite{vinen-2001}}\hspace{0pt}%DIFAUXCMD
.
In }\DIFaddend 2D\DIFaddbegin \DIFadd{, similarly to }\DIFaddend classical turbulence, 
...
\DIFdelbegin \DIFdel{Reconnections occur continuously in turbulence: they take place when
two vortex lines collide and recombine, exchanging heads and tails, 
altering the overall topology of the flow
\mbox{%DIFAUXCMD
\cite{koplik-levine-1993,bewley-etal-2008,rorai-etal-2016,serafini-etal-2017,galantucci-baggaley-parker-barenghi-2019,villoisUniversalNonuniversalAspects2017,villois2020irreversible}}\hspace{0pt}%DIFAUXCMD
. 
}\DIFdelend %DIF > \red{The onset and development of quantum turbulence is fundamentally driven by such reconnections, which facilitates energy transfer and the breakdown of coherent vortex structures.}  
%DIF > Reconnections occur continuously in turbulence: they take place when
%DIF > two vortex lines collide and recombine, exchanging heads and tails, 
%DIF > altering the overall topology of the flow
%DIF > \cite{koplik-levine-1993,bewley-etal-2008,rorai-etal-2016,serafini-etal-2017,galantucci-baggaley-parker-barenghi-2019,villoisUniversalNonuniversalAspects2017,villois2020irreversible}. 
...
\DIFdelbegin \DIFdel{. }\DIFdelend %DIF > \red{which is not globally conserved across the reconnection event}. 
...
%DIF < Punctuated energy injection resulting from the violent nature of vortex reconnections \cite{stasiak2024quantum} pave the way for an imbalance of chirality by multi-scale energy injection.
%DIF > Punctuated energy injection resulting from the violent nature of vortex reconnections \cite{stasiak2025experimental} pave the way for an imbalance of chirality by multi-scale energy injection.
%
...
\DIFaddbegin \DIFadd{All results reported hereafter refer only to the reconnection of one pair of vortices. 
%DIF > \red{In our simulations, the reconnections of the two pairs of vortices occur at different times; here we consider only the vortex dynamics up to and shortly after the first reconnection (see Fig.~\ref{fig:total-helicity}). Hence, all results reported and discussed herafter refer to the first reconnection.}. 
}\DIFaddend 
...
\cite{stasiak2024quantum}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{stasiak2025experimental}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend . The second row shows the rendering of the local helicity $H(\x)=\v_n\cdot\bom$, where we observe a clear local helicity production, with an abrupt change of sign due to the rearrangement of the vortex topology. Remarkably, during reconnection there is a net 
...
\DIFaddendFL \caption{Temporal evolution of the normal fluid helicity $\mathcal{H} = \int_{\mathcal{V}}H(\x)dV$ computed over the entire volume $\mathcal{V}$. For superfluid helium, it is proper to make the helicity dimensionless in terms of $\kappa^2$. \DIFdelbeginFL \DIFdelFL{The quantities $E_R$ and $L_R$ are the energy $E$ and integral length scales $L_0$ of the normal fluid at reconnection time $t_R$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\tau_R = L_R/E^{1/2}_R$}\DIFaddendFL .
%DIF > \red{The 3D rendering shows the asynchronous nature of the reconnections in the initial configuration, with the two reconnections occurring at different times; same colour scheme as in Fig.~\ref{fig:visualisation}.}
}
...
\caption{\DIFaddbeginFL \emph{\DIFaddFL{(a):}} \DIFaddFL{Normal fluid kinetic energy spectrum $E(k)$ before 
reconnection (dashed lines), at reconnection (solid lines) 
and after reconnection (dotted lines) for $T=1.9K$ (red)
and $T=2.1K$ (blue).}\emph{\DIFaddFL{(b):}} \DIFaddFL{Spectral normal fluid kinetic energy flux, $\Pi (k)$. It is normalised using the integral scale and the normal fluid energy at reconnection.
}\emph{\DIFaddFL{Inset:}} \DIFaddFL{Post reconnection evolution of the integral length scale, $L$.
Times and temperatures are labelled as in Fig.~3a.}\DIFaddendFL }
\DIFdelbeginFL %DIFDELCMD < \label{fig:energy-flux}
%DIFDELCMD <     \end{subfigure}
%DIFDELCMD <     %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\emph{\DIFdelFL{(a):}} %DIFAUXCMD
\DIFdelFL{Normal fluid kinetic energy spectrum $E(k)$ before 
reconnection (dashed lines), at reconnection (solid lines) 
and after reconnection (dotted lines) for $T=1.9K$ (red)
and $T=2.1K$ (blue).}\emph{\DIFdelFL{(b):}} %DIFAUXCMD
\DIFdelFL{Spectral normal fluid kinetic energy flux, $\Pi (k)$. It is normalised using the integral scale and the normal fluid energy at reconnection.
}\emph{\DIFdelFL{Inset:}} %DIFAUXCMD
\DIFdelFL{Post reconnection evolution of the integral length scale, $L_0$.
...
This behaviour is quantified by the evolution of the integral length scale \DIFdelbegin \DIFdel{$L_0$}\DIFdelend \DIFaddbegin \DIFadd{$L$}\DIFaddend , defined as
\begin{equation}
    L \DIFdelbegin \DIFdel{_0 }\DIFdelend = \frac{\pi}{2 E}\int_0^{\infty}\frac{E(k)}{k}dk, %\Big/ \int_0^{\infty}\!\!E(k)dk.
\end{equation}
The inset of Fig.~\DIFdelbegin \DIFdel{\ref{fig:energy-flux} shows that
$L_0$ }\DIFdelend \DIFaddbegin \DIFadd{3b shows that
$L$ }\DIFaddend indeed  increases steadily in the post-reconnection regime. Note that times have been normalised by the largest eddy-turnover-time at the reconnection event, evidencing  it fast evolution.

To explain the inverse energy transfer shown in Fig.~\DIFdelbegin \DIFdel{\ref{fig:kinetic-energy}}\DIFdelend \DIFaddbegin \DIFadd{3b}\DIFaddend ,
...
\DIFaddbegin \DIFadd{For the sake of completeness, it is worth noting that the total helicity of the flow (obtained by adding the superfluid helicity to that of the normal fluid $\mathcal{H}$) is not conserved.
}\DIFaddend From Fig.~\ref{fig:mutual-friction-decomp}\DIFaddbegin \DIFadd{, we consider a time $t^*$ after reconnection where the ratio $|f^+|^2/|f^-|^2$ has sufficiently decayed to a roughly 5\% difference and }\DIFaddend we determine the non-dimensional timescale \DIFdelbegin \DIFdel{$\tau = (t^*-t_R)E_R^{1/2}/L_R$
}\DIFdelend \DIFaddbegin \DIFadd{$\tau = (t^*-t_R)/\tau_R$ }\DIFaddend during which the mutual friction force is chiral as a result of reconnections: $\tau \approx 0.01$ and $\tau \approx 0.005$
...
vortex line density $\mathcal{L}$ (length of vortices per unit volume) is larger than $10^8 \text{m}^{-2}$ \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{stasiak2024quantum,barenghi2004}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{stasiak2025experimental,barenghi2004}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend ,
...
energy in the normal fluid \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{stasiak2024quantum}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{stasiak2025experimental}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend , but also triggers in the normal fluid
...
This inverse energy transfer \DIFdelbegin \DIFdel{arises from }\DIFdelend \DIFaddbegin \DIFadd{is triggered by }\DIFaddend the helical character of the
\DIFdelbegin \DIFdel{friction generated by }\DIFdelend \DIFaddbegin \DIFadd{mutual friction, directly arising from }\DIFaddend the Kelvin waves released by the reconnecting
...
finite temperature superfluids\DIFaddbegin \DIFadd{, where vortex reconnections play a key role}\DIFaddend . 
In circumstances where the vortex density 
$\mathcal{L}$ is large and where the isotropy of the vortex tangle 
is broken by external forcing \DIFaddbegin \DIFadd{(for instance thermal counterflows~\mbox{%DIFAUXCMD
\cite{vinen-1957a}}\hspace{0pt}%DIFAUXCMD
)}\DIFaddend , 
the chirality of the flow generated by the 
frequent \DIFaddbegin \DIFadd{non-symmetrical }\DIFaddend reconnections may be strong enough to induce an inverse energy cascade\DIFaddbegin \DIFadd{~}\DIFaddend \cite{notes_recon}.
...
The microscopic mechanism that we have described \DIFdelbegin \DIFdel{is probably what
triggers }\DIFdelend \DIFaddbegin \DIFadd{may be relevant in triggering
}\DIFaddend the inverse energy cascade which is observed numerically in  
large-scale simulations of counterflow turbulence at large heat fluxes \cite{polanco2020}, which is indeed inherently not isotropic. \DIFaddbegin \DIFadd{However, formalising the connection between high counterflow, unbalanced helicity injection and inverse energy cascade is  challenging, as the model used in \mbox{%DIFAUXCMD
\cite{polanco2020} }\hspace{0pt}%DIFAUXCMD
and the one used in our work describe superfluid helium at very different scales.
}\DIFaddend %, hence triggering non-fully reconnections. 
...
%DIF <  \bibliography{references}% Produces the bibliography via BibTeX
